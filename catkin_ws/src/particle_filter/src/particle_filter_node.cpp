...
sensor_msgs::Imu *imu; 
double imu_yaw; 
bool imu_reading_received = false; 

//Keeps GPS reading. 
particle_filter::Pose *gps_pose = 0; 
bool gps_reading_received = false; 

bool can_start() {
	return (odom && gps_pose && imu && pf->calibrated()); 
}
...
void gps_callback(particle_filter::Pose gps_reading) {
	...
	//Sets flag letting system know a new reading is available. 
	gps_reading_received = true;
...
void weigh_using_gps_and_imu(Particle *particle, void **args) {
	...
	if (0)
		...
		//Sets flag to know that reading has been read. 
		gps_reading_received = false; 
	}
...
int main(int argc, char **argv) {
	...
	pf = new ParticleFilter(200, &weigh_using_gps_and_imu); 
